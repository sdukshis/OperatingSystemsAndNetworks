/* Avoid deadlock by ordering resources */
...
void swap_mtx(pthread_mutex_t **a, pthread_mutex_t **b);
...
void exec_transaction(struct deposit *from,
                      struct deposit *to,
                      int amount) {
    pthread_mutex_t *fst = &from->mutex;
    pthread_mutex_t *snd = &to->mutex;

    if (from > to) {
        swap_mtx(&fst, &snd);
    }
    exit_on_error(pthread_mutex_lock(fst), "pthread_mutex_lock");
    usleep(rand() % 10);
    exit_on_error(pthread_mutex_lock(snd), "pthread_mutex_lock");
    
    from->balance -= amount;
    to->balance += amount;
    
    exit_on_error(pthread_mutex_unlock(snd), "pthread_mutex_unlock");
    exit_on_error(pthread_mutex_unlock(fst), "pthread_mutex_unlock");
}
